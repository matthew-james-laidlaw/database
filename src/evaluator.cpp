...
auto Evaluator::Evaluate(InsertStmt const& stmt) -> void
{
	...
	if (inserted == 0)
	{
		LogInfo("inserted 0 item(s): duplicate key: '{}'", stmt.GetKey().GetLexeme());
	}
	else
	{
		LogInfo("inserted 1 item(s): key: '{}', value: '{}'", stmt.GetKey().GetLexeme(), stmt.GetValue().GetLexeme());
	}
}
...
auto Evaluator::Evaluate(SelectStmt const& stmt) -> void
{
	...
	if (selected)
	{
		LogInfo("selected 1 item(s): key: '{}', value: '{}'", stmt.GetKey().GetLexeme(), *selected);
	}
	else
	{
		LogInfo("selected 0 item(s): nonexistent key: '{}'", stmt.GetKey().GetLexeme());
	}
}
...
auto Evaluator::Evaluate(UpdateStmt const& stmt) -> void
{
	...
	if (updated == 0)
	{
		LogInfo("updated 0 item(s): nonexistent key: '{}'", stmt.GetKey().GetLexeme());
	}
	else
	{
		LogInfo("updated 1 item(s): key: '{}', value: '{}'", stmt.GetKey().GetLexeme(), stmt.GetValue().GetLexeme());
	}
}
...
auto Evaluator::Evaluate(DeleteStmt const& stmt) -> void
{
	...
	if (deleted == 0)
	{
		LogInfo("deleted 0 item(s): nonexistent key: '{}'", stmt.GetKey().GetLexeme());
	}
	else
	{
		LogInfo("deleted 1 item(s): key: '{}'", stmt.GetKey().GetLexeme());
	}
}
